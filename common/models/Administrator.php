...
class Administrator extends ActiveRecord implements IdentityInterface
{
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if(!empty($this->roles)){
            ...
            foreach($this->roles as $role){
                ...
                Yii::$app->authManager->assign($authitem, $this->id);
            }
        }
    ...
    public function getConfigs(){
        ...
        if($this->config_id) {
            $query->where(['in', 'id', $this->config_id]);
        }
        return $query;
    }

    public function getConfigNames(){
        if(empty($this->config_id)) return '全部';
        $names = $this->getConfigs()->select('name')->column();
        return implode('/', $names);
    }
}
